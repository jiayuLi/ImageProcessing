...
public class Geo_ProcessMaskData extends Component implements ActionListener {

	String descs[] = { "Original", "placeholder", "placeholder",
			"placeholder", "Ranged-Geodesic/Mean", "Ranged-Geodesic/Median",
			"Ranged-Geodesic/0.5*(Mean+orig)", "placeholder" };
		...
		case 5: /* geoC + median */{
			...
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++) {
					...
					// find the median pixel of the mask
					Collections.sort(pixStorage);
					Pixel resultPix = pixStorage.get(pixStorage.size()/2);
					biFiltered.setRGB(j, i, getIntFromColor(resultPix.getColor()));
		...
		case 6: /* RGEO_1/2(Mean+Orig) */{
			...
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++) {
					...
					sumR = (sumR + sourcePixArray[i][j].getColor().getRed())/2;
					sumG = (sumG + sourcePixArray[i][j].getColor().getGreen())/2;
					sumB = (sumB + sourcePixArray[i][j].getColor().getBlue())/2;
	...
	public static void main(String s[]) {
		...
		// p1 = new JPanel();
		final Geo_ProcessMaskData si = new Geo_ProcessMaskData();
		// p1.add(si);
		f.add("Center", si);
