//----------------------------------
// InterpolationSmoothTesting.java
// A later, optimized implementation on geodesic filtering
// Author: Jiayu Li
//----------------------------------
...
public class PixelMigrationSmoothTesting extends Component implements ActionListener {
	...
	String descs[] = { "Original", "bicubic upsampled", "spring mass", "1D spring mass", "Length distribution", "Diagonal Spring", "angular spring",  "pre-filtering + angular"
			,"down-up, angular","springLengthSmoothing","gradient angular", "angular 13x13 smooth weak spring length"};

	static String IMAGESOURCE = "testImg141219/all/2.jpg";
	...
	public PixelMigrationSmoothTesting() {
		try {
	...
	private Vec2 calcForceBetweenTwoPixelAngularSpringSuccessAttempt13x13(
		...
		for(int i = topleftPix.getY()-7; i<topleftPix.getY()+7; i++){
			for(int j= topleftPix.getX()-6; j<topleftPix.getX()+7; j++){
		...
		*/
		if((center.getX()>=142&&center.getY()>=89&&center.getX()<=297&&center.getY()<=169)
				||(center.getX()>=82&&center.getY()>=295&&center.getX()<=126&&center.getY()<=344)
				||(center.getX()>=263&&center.getY()>=332&&center.getX()<=333&&center.getY()<=444)){
	...
	*/
	...
	public static void main(String s[]) {
		...
		// p1 = new JPanel();
		final PixelMigrationSmoothTesting si = new PixelMigrationSmoothTesting();
		// p1.add(si);
		f.add("Center", si);
