//two different filters, range-geo and exponential-geo
...
public class GeodesicFilterV1 extends Component implements ActionListener {
	...
	public void filterImage() throws IOException {
		...
		case 1: {/* geodesic wrong / residual enhanced */
			...
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++) {
					...
					Color residual = colorSub(sourcePixArray[i][j].getColor(), tempColor);
					
					Color enhanced = colorPlus(sourcePixArray[i][j].getColor(), residual);

					biFiltered.setRGB(j, i, getIntFromColor(enhanced));
					if (jcb.isSelected()) {
						i = h;
						j = w;
					}
		...
		case 2: {/* geodesic wrong residual */
			biFiltered = ImageIO.read(new File(IMAGESOURCE));
			int[][] intPixArray = convertTo2DUsingGetRGB(bi);
			Pixel[][] sourcePixArray = convert2DIntArrayToPixelArray(intPixArray);
			Pixel[][] resultPixArray = new Pixel[h][w];
			...
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++) {
					...
					biFiltered.setRGB(j, i, getIntFromColor(residual));
					if (jcb.isSelected()) {
						i = h;
						j = w;
					}
						...
						if (temp.getX() > 0) {
							...
							if (!bottom.isUsedByCurrentMask()) { // dr*dr +
								...
								double colorDistance = (bottom.getColor()
										...
										* (bottom.getColor().getBlue() - temp
												.getColor().getBlue());
								colorDistance = temp.getWeight() + R * Math.pow(colorDistance, exponentialPower/2);
								if (bottom.isUsedByCurrentPQ()) {
									if (colorDistance < bottom.getWeight())
										bottom.setWeight((int) colorDistance);
									else
		...
		case 6: /* upsampling + geodesic + original pixel */{
			w = bi.getWidth(null);
			h = bi.getHeight(null);
			...
			// background filter
			Pixel[][] sparseUpsampledSourcePixArray = new Pixel[(int) (h * resizeScale)][(int) (w * resizeScale)];
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++) {
					sparseUpsampledSourcePixArray[(int) (i * resizeScale)][(int) (j * resizeScale)] = sourcePixArray1[i][j];
				}
			}
			biFiltered = resize(bi, (int) (w * resizeScale),
					(int) (h * resizeScale),
			...
			Pixel[][] resultPixArray2 = new Pixel[h][w];
			LinkedList<Pixel> pixStorage2 = new LinkedList<Pixel>();
			LinkedList<Pixel> upsampledPixStorage = new LinkedList<Pixel>();
			...
			for (int i = 0; i < h * resizeScale; i++) {
				for (int j = 0; j < w * resizeScale; j++) {
					...
					maskPriorityQueue2.add(sourcePixArray2[i][j]);
					int origPixCount = 0;
					while (origPixCount < upsampleMaskSize) {
						// get the lowest weighted pixel from the priority queue
						Pixel temp = maskPriorityQueue2.remove();
						...
						// the one on the bottom
						if (temp.getX() > 0) {
							Pixel bottom = sourcePixArray2[temp.getY()][temp
									.getX() - 1];
						...
						if (sparseUpsampledSourcePixArray[temp.getY()][temp
								.getX()] != null) {
							upsampledPixStorage
									.add(sparseUpsampledSourcePixArray[temp
											.getY()][temp.getX()]);
							// System.out.println("*sparse added:"+ temp.getY()
							// + " " + temp.getX());
							origPixCount++;
						}
					}
					...
					// calculate the average color of the mask
					int sumR = 0, sumG = 0, sumB = 0, localMaskSize = 0;
					while (!pixStorage2.isEmpty()) {
						Pixel tempPix = pixStorage2.remove();
						tempPix.setWeight(999999999);
						tempPix.setUsedByCurrentMask(false);
					...
					while (!upsampledPixStorage.isEmpty()) {
						Pixel tempPix = upsampledPixStorage.remove();
						sumR += tempPix.getColor().getRed();
						sumG += tempPix.getColor().getGreen();
						sumB += tempPix.getColor().getBlue();
					}
					sumR /= upsampleMaskSize;
					sumG /= upsampleMaskSize;
					sumB /= upsampleMaskSize;
			...
			System.out.println("average mask size" + (double) sumMaskSize
					/ (w * h * resizeScale * resizeScale));
			break;
		}
	...
	public static void main(String s[]) {
		...
		// p1 = new JPanel();
		final GeodesicFilterV1 si = new GeodesicFilterV1();
		// p1.add(si);
		f.add("Center", si);
